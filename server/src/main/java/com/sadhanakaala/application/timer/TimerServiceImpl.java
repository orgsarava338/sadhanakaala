...
import com.sadhanakaala.api.dto.mapper.TimerMapper;
...
import com.sadhanakaala.domain.timer.Segment;
...
public class TimerServiceImpl implements TimerService {
    ...
    public Timer createTimer(@NonNull TimerDTO timerDTO) {
        ...
        if (timerEntity.getUsageCount() < 0) timerEntity.setUsageCount(0);
    ...
    public Timer updateTimer(@NonNull String id, @NonNull TimerDTO updates, @NonNull String actorUserId) {
        ...
        TimerEntity existing = timerRepository.findById(id).orElseThrow(() -> new NotFoundException("Timer not found: " + id));
    ...
    public Timer getTimer(@NonNull String id) {
        return timerMapper.toResponse(timerRepository.findById(id).orElseThrow(() -> new NotFoundException("Timer not found: " + id)));
    }
    ...
    public Timer incrementUsageAndTouch(@NonNull String timerId) {
        ...
        return timerMapper.toResponse(timerRepository.findById(timerId).orElseThrow(() -> new NotFoundException("Timer not found after increment: " + timerId)));
    }
}